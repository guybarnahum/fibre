...
The motivating case is ordinary life: a Thread has an encounter, notices that something about it remains unresolved, and continues to think about it later. The Thread may talk with someone, read, search the internet, compare the experience with older memories, or simply let time pass. The experience may become provisionally settled, remain open, or later reopen when new experience changes the interpretive situation.
...
`settled` means only that the Thread currently has no active interpretive question about the experience. It does **not** mean Fibre has determined the correct interpretation, that no contradiction remains, or that the experience can never matter again.
...
The Thread is not required to settle every experience. Some ambiguity may persist for years; some issues may lose relevance without a clean answer.
...
Interpretive settlement should be append-only and revisable.

A settlement record may say, in natural language, what is no longer actively unresolved and why the Thread currently considers the issue sufficiently understood.
...
```text
prior unsettled/settled record
    + new evidence refs
    -> reopened interpretive question
```

Fibre must not periodically reopen settled experiences merely to simulate depth.
...
This is the simplest case. The memory can carry an unsettled interpretive question until meaning forms, remains absent, or the Thread stops pursuing the question.
...
3. **Settlement is provisional personal closure, not objective truth.** A Thread may stop actively processing an experience while remaining corrigible.
4. **Settled meaning may reopen under new evidence.** Continuity requires preserving prior interpretations while allowing later experience to disturb them.
...
3. Should a memory be allowed to remain on the interpretive agenda indefinitely?
4. What distinguishes healthy continuing reflection from a pathological or mechanically amplified rumination loop?
5. Can settled remembered meaning reopen automatically when relevant contradictory evidence is admitted, or must a Thread first notice that evidence in cognition?
6. How should another person's conflicting memory of a shared event affect the Thread without corrupting autobiographical authority?
7. Can an initially `not_remembered` event later become autobiographical memory, and if so, what is the provenance of that later retention?
8. When internet research is motivated by a private unresolved experience, what query/disclosure boundary prevents private Thread state from being leaked to external services?
9. Should public `onMyMind` content require explicit Thread authorization rather than ordinary presentation projection?
