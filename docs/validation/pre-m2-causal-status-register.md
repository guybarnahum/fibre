...
The first **live model response** seals the evidentiary cycle. A missing credential or other pre-invocation block does not.

The accepted operational distinction is now explicit in [`semantic-guardian-v3-freeze.md`](semantic-guardian-v3-freeze.md):

- an operational provider/transport/protocol attempt that produces no authoritative judgment may be retried up to the predeclared cap of `2` retries within the same trial slot;
- only the eventual authoritative judgment counts toward `k = 5`;
- every operational attempt is retained in the sealed evidence artifact;
- exhausting all `3` attempts for a required trial leaves the judgment missing and fails the cycle;
- a parseable cognition result that fails Fibre semantic validation, misses a predeclared behavioral expectation, or violates a robustness/grounding condition is **not retried** and fails the cycle;
- a harness/code failure after live execution begins also fails and seals the cycle.

After any sealed cycle failure, fixes or evaluation changes require a new frozen boundary and new held-out acceptance set. There is no post-result “clearly unrelated failure” exception.