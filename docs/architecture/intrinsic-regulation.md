...
### Situated Percept is not PerceptFrame

[Situated Percept](situated-perception-and-salience.md) and the regulator's `PerceptFrame` sit at different layers.

```text
Situated Percept
  bounded exterior scene available to ordinary cognition
  setting / observable entities / activities / admitted happenings

PerceptFrame
  bounded lower-level control/sensory evidence
  internal state / environment / social cues / evidence refs
```

They may be derived from some of the same World facts, but neither should be generated by copying the other's semantic conclusions.

For example, a Situated Percept may say that a familiar Thread is sitting nearby sketching. A PerceptFrame may contain grounded proximity/contact cues. Neither may say `she wants company` or `I am attracted to her`.

The Salience Gate may consume both exterior opportunity facts and current regulator pressure while remaining mechanical. Interior Cognition receives the meaningful exterior concern plus any properly interocepted semantic state; raw regulator labels do not become persona instructions.
