...
The rule is accepted and now executable for authoritative World relational state.

Already behind `InfraDriver`:

- authoritative World/Thread, Genesis, Civil Registry, identity, memory, situated-life, embodiment, runtime, obligation and expression stores through `InfraDriver.state`;
- local SQLite transactional state with the Fibre-required relational/transaction guarantees;
- Cloudflare SQLite-backed Durable Object transactional state with the same guarantees;
- provider-neutral World scheduling through the same `InfraDriver.scheduler` `get/schedule/cancel` contract locally and on Cloudflare, with Durable Object alarms as the cloud mechanism;
- Cloudflare World runtime composition that colocates the logical World `state + scheduler` scope in one SQLite-backed Durable Object and uses service bindings for existing fetch-shaped downstream boundaries;
...
1. **The durable model-invocation journal writes a local filesystem journal directly.** It remains an explicit migration exception until a suitable Infra capability is adopted.
2. **Cloud runtime composition is incomplete.** World and Birth Center still run as local Node deployments; provider-neutral World persistence does not by itself provide durable cloud scheduling, reconciliation or Worker composition.
...
The state-port proof now uses the hardest transaction shape Fibre already needs rather than a toy key/value wrapper:

> **The executable `infra.state` contract preserves authoritative World relational transactions across local SQLite and Cloudflare SQLite-backed Durable Object state.**
...
A local SQLite implementation should remain available for deterministic development and tests. The Cloudflare mapping is SQLite-backed Durable Object storage and must continue satisfying the same conformance and real-World-store contract as local SQLite. The Fibre guarantee, not a vendor product name, is the contract.
...
The migration order is driven by real transaction requirements. The transactional-state proof and World persistence portability slice established the shared state boundary for the authoritative World consistency domain. Runtime cloud scheduling/composition, Birth Center provisional-state migration and the durable invocation journal remain later work as their concrete capability requirements become executable.
