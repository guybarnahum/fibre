...
Operator maps are projections of this authority, not a new place authority. A Threads world map uses CurrentSituation and its resolved place reference whenever enacted present exists. Population-scale inspection should resolve current situations and their referenced place evidence set-wise from World state rather than issuing per-Thread reads or causing LivedNow cognition. Latitude/longitude may project that authoritative place onto a geographic map, but coordinate coincidence is never evidence of physical co-presence and birthplace is never current presence. To keep every living Thread inspectable before its first enacted CurrentSituation, an operator UI may plot canonical birthplace only as an explicitly marked **awaiting LivedNow** placeholder. That placeholder carries no co-presence or present-time authority and must disappear as soon as CurrentSituation exists.
