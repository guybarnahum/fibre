...
**Current slice: compact Interior Cognition core.** The first isolated component now owns bounded private-context selection and one model-backed private judgment boundary without owning any domain state. Its focused semantic proof shows the same concern can be bent differently by distinct autobiographical remembered meaning while raw genome remains absent from ordinary cognition. This is a wiring/authority proof, not yet live model-quality acceptance; Flight Plan is the next intended real consumer after the component is green. Do not synthesize generic semantic state merely because the current store is empty, and do not touch Commons behavior until the person-level cognition seam is proven.
