...
**Current slice: Commons Entry Interior Cognition.** Migrate only the bounded Fibre Commons `enter | stay_out` decision behind the same shared private-mind component. Commons supplies its concrete affordance/current-life question; Interior Cognition owns private evidence selection and reconciliation. Raw genome and a domain-owned persona prompt must disappear from this decision. Preserve the existing World authority boundary: entering may author mediated presence while staying out remains a real voluntary outcome. Prove with one organism-level differential that persisted lived meaning can bend Commons entry without moving anyone or manufacturing a meeting. Do not yet migrate meeting initiation or invitee `accept | decline | defer`; those are the next social adapters after Commons entry is grounded.
