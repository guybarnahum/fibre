...
**Current slice: Social Initiation Interior Cognition.** Migrate only the initiator-owned `initiate | not_initiate` judgment behind the shared private-mind component. Co-present counterparties/current LivedNow are external facts; Interior Cognition alone selects private identity, semantic state, autobiographical memory and relationship evidence. Remove raw genome/persona assembly from this decision. Keep the outward request concrete and separately inspectable, and preserve `not_initiate` as a valid voluntary outcome. Do not yet migrate invitee `accept | decline | defer`; that remains the next adapter after initiation is grounded.
