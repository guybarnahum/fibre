...
**Current slice: read-only Interior Baseline.** Before implementing the component, run `npm run interior-cognition:baseline:staging` against the exact deployed SHA. It inspects the authoritative symbolic genome, identity, semantic state, autobiographical memory, relationships, neutral published shell and currently stored LivedNow without invoking models or mutating life. The result decides whether Slice 2 is primarily a cognition-wiring repair or whether missing developmental evidence must be addressed first.
