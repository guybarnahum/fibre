...
**Lived Planning — CLOSED locally.** Personal Flight Plan is now the first real domain adapter over Interior Cognition. The planner no longer assembles a parallel person prompt: Fibre-owned context selection supplies bounded authoritative identity, semantic state, autobiographical remembered meaning and relationship evidence. The focused organism-level proof holds World conditions constant and shows different persisted remembered meaning bending ordinary intended life into different attributable plans. Full repository validation is green.\n\n**Current slice: Lived Planning Live Proof.** Deploy the exact clean SHA and run `npm run lived-planning:staging`. The probe uses existing private Observatory authority plus the enacted `CurrentSituation.evidenceRefs`; it does not add a new runtime endpoint or force replanning. It closes only when at least two real current staging lives cite different autobiographical-memory or relationship evidence through the admitted planning path and produce distinguishable current intended-life moments. This is a deployed integration/attribution proof, not a substitute for the controlled causal differential above. Commons/social behavior remains untouched until this proof is grounded.
