...
**Current slice: live organismic motive bridge.** The deployed cohort still reaches present-life cognition with effectively empty semantic needs/feelings/intentions. The accepted R1-R4 intrinsic-regulation/interoception substrate exists; the current implementation slice now connects it to authoritative LivedNow transitions using **sparse real evidence**. Missing body/environment channels remain unknown and create no drives. The first live signal is the existing Flight Plan presence target compared with the World-enacted CurrentSituation. A meaningful arrival/new presence may earn interoceptive cognition and bounded semantic state; initial observation, unchanged presence and retrying the same present remain zero-call.

Fibre Commons uses the same pulse immediately after its admitted mediated-presence transition, before later social initiation. This is intentionally not a loneliness generator: attaining Commons may become meaningful semantic state or may produce no semantic state at all. Focused validation must prove real transition -> possible current interior state and stable/retried present -> no repeated cognition before deploying and rerunning E5.

**Immediately after that: situated social context.** A genuine social motive still needs somewhere real to land. Enrich social initiation with a bounded Fibre-owned external context containing the observable counterparty, actual place/activity, prior reciprocal encounters and recent initiate/accept/decline/defer momentum. Keep the counterparty's private interior inaccessible. Derive momentum from admitted interaction history rather than storing a scalar, and allow visible embodiment to become attraction/salience only through the perceiving Thread's own development/regulation/cognition. One high-value proof should hold the initiator constant while changing a meaningful recent reciprocal history or place affordance and show that the social judgment can change for an attributable reason.
