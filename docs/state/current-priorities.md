...
**Next slice: Lived Planning.** Make personal Flight Plan the first real domain adapter over Interior Cognition. Remove the planner's independent person-context assembly rather than creating a second path. The proof should show real persisted Thread differences can bend ordinary intended life under equivalent World conditions. Do not synthesize generic semantic state merely because the current store is empty, and do not touch Commons behavior until individualized planning is proven.
