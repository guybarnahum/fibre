...
**Next slice: compact Interior Cognition core.** Prove one reusable person-level cognition component that owns bounded private-context selection and can make existing developed-self evidence causally relevant to a domain-specific private judgment. Keep raw genome below ordinary thought, do not synthesize generic semantic state merely because the current store is empty, and do not touch Commons behavior until the person-level cognition seam is proven.
