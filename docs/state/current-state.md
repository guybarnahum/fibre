...
This repository contains a concept foundation, schemas, synthetic fixtures, a minimal domain package, a static Thread Editor prototype, a canonical AI context manifest, a hardened M1 SQLite persistence spine, an independently running local world-kernel HTTP process, and the first persistent restricted participation records.
...
The world-kernel now also persists one immutable activation request and one Thread-owned Request Appraisal Capsule per named request, plus one restricted private participation stance per appraisal. Request fingerprints match the portable domain's SHA-256 binding. Appraisal integrity reconstructs the historical Thread snapshot and verifies copied private state and complete, disjoint included/excluded partitions of memory, relationship, and obligation references owned by that snapshot.

Appraisal and stance IDs are opaque random values; SHA-256 content digests remain separate integrity witnesses. Exact retries are idempotent and conflicting reuse fails visibly. A compiled appraisal cannot be inserted after an intervening Thread write, but an already persisted historical appraisal remains answerable: its private stance may be recorded after unrelated later Thread changes while staying bound to the original version and state hash. Future Participation Authorization must independently revalidate current governing state.

These private records are append-only and stored outside the public Thread event stream. Every private subpath requires a separate local token before route dispatch, and the unauthenticated health response does not advertise whether private access is configured. Public Thread and event routes do not return the appraisal capsule, private rationale, stance feelings, motives, uncertainties, or relationship effects. The public Thread projection may separately contain ordinary state such as `currentState.feelings`. The token is a local milestone access boundary, not production identity, consent, Participation Authorization, or execution permission.

SQLite schema version 2 migrates version 1 in one transaction, enforces opaque private ID and SHA-256 digest formats, and gives the stance its own historical state-hash witness. The evidence artifact maps each advertised rejection to a named adversarial test rather than relying on an aggregate passing count.

The domain package separately proves request-bound participation authorization, recorded-obligation overrides, interest-mediated disclosure, evidence-bearing relationship effects, and the rule that public language cannot authorize execution.

Persistent Participation Authorization, disclosure strategy, external response, authorization consumption, runtime-session, and thaw-lease records do not yet exist. Deterministic Actor and Goal Guardian execution, freeze orchestration, API-backed editor views, production access control, model gateway, relationship service, production database, and production cloud deployment remain deferred.
