...
**W3 cheap general Salience Gate is now implemented pending repository validation.** One compact pure gate runs before social-initiation cognition. It does not score attraction, sociability or willingness and does not call a model. For the first co-present-Thread consumer, grounded anchors include shared mediated context, planned/enacted participant presence, recent admitted interaction history, and a producer-grounded `unexpected_observable` cue for context-relative scene anomalies. Anomaly means only “unusual enough here to enter attention”; interpretation remains downstream.

Without a materiality anchor, compatible co-presence remains background. Background is not `not_initiate`: no private decision occurred and no social history exists. The W3 organism proof requires exactly that distinction: compatible ambient co-presence with no anchor returns background, `initiation:null`, zero model calls, no reciprocal interaction record and no Encounter Story. Salient opportunities continue into Interior Cognition, which remains free to choose `not_initiate`. After validation, proceed to **W4 ordinary World opportunities from active LivedNow**, which must ground anomaly cues from actual scene facts rather than test-authored labels. Then continue toward natural lived encounter acceptance:
