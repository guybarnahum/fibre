...
The current prerequisite slice is **Commons Entry Interior Cognition**: route only voluntary `enter | stay_out` ambient-presence judgment through the same private-mind component, preserving physical life and keeping Commons presence distinct from social initiation or consent. After Commons entry is grounded, migrate social initiation and invitee stance behind the same boundary, then resume full E5 acceptance:
