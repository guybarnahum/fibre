...
The current prerequisite is therefore the **live organismic motive bridge**. The accepted intrinsic-regulation/interoception machinery already exists. The current implementation connects it to LivedNow using a sparse evidence rule: absent sensor/body channels remain unknown, while the already-authoritative personal Flight Plan presence target and enacted CurrentSituation can produce a RegulationFrame. A material transition such as arrival or newly attained mediated presence may earn interoceptive interpretation and bounded semantic state; a first observation or unchanged/retried present must not invent a transition or spend a cognition call. Fibre Commons runs this same pulse immediately after an admitted entry so later social initiation can see any state the actual transition caused.

This remains an implementation slice until focused tests, `npm run slice:validate`, and deployed staging evidence pass. It must not synthesize generic loneliness, fatigue or emotion, and invitee stance remains deferred until motive plus situated social context are credible.

The next social-context refinement is already fixed architecturally: social initiation must eventually reconcile that motive with the **specific counterparty, prior shared encounters, recent reciprocal initiation/accept/decline/defer momentum, visible embodiment where legitimately observable, and the actual place/activity affordance**. Social momentum is derived recent history, not a durable score; private `not_initiate` thoughts are not shared history; the other Thread's private interior remains inaccessible. Only after motive plus situated social context are credible should Fibre expect natural accepted social stories in staging. Then resume full E5 acceptance:
