...
**W2 direct social response through Interior Cognition is now implemented pending repository validation.** The remaining legacy `accept | decline | defer` prompt no longer assembles raw genome traits, self-description, manually filtered relationships, semantic state or memories. The recipient receives the concrete outward request + Situated Percept + remaining Flight Plan as exterior context; the shared Interior Cognition component selects her private developed-self evidence. Politeness, reciprocity, culture and relationship expectations may matter only insofar as grounded life evidence makes them matter; none is an acceptance rule. Proposed defer times remain checked mechanically against the real plan horizon.

The W2 proof holds the exact outward request, recipient and private selected evidence constant while changing only a structured observable setting fact; that change can bend `accept | decline`. After validation, proceed to **W3 cheap general Salience Gate**. Then continue toward natural lived encounter acceptance:
