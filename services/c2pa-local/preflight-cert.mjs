...
export function validateLocalC2paCertificateChain(input) {
  ...
  if (blocks.length < 2) {
    throw new Error(
      "local C2PA credentials use a legacy/self-signed certificate; regenerate them with services/c2pa-local/generate-dev-cert.sh",
    );
  }
...
async function main() {
  ...
  try {
    ...
    throw new Error(
      `Unable to load local C2PA certificate chain. Run services/c2pa-local/generate-dev-cert.sh first. ${error.message}`,
    );
  }
