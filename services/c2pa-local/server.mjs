...
// LocalSigner currently requires ES256 keys as unencrypted PKCS#8 PEM. Node's
// crypto parser accepts both the PKCS#8 keys generated by the current helper and
// older SEC1 "EC PRIVATE KEY" files, so normalize in memory before crossing the
// C2PA SDK boundary. This keeps already-created local development credentials usable.
const privateKey = createPrivateKey(privateKeyInput).export({ format: "pem", type: "pkcs8" });
