...
The bundled font bytes are part of the versioned credential template. Human-readable identity values preserve the authorized spelling and case; only labels explicitly request uppercase. Each text field also owns a hard presentation width so a credential fails visibly rather than clipping or shrinking identity material unpredictably. The local cutter and Cloudflare FIA deployment both load these exact TTF assets; neither relies on an operating-system or browser font stack.
