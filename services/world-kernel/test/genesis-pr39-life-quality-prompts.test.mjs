...
import { GENESIS_REPLACEMENT_PASS_A_PROMPT } from "../src/genesis-replacement-pass-a.mjs";
import {
...
test("PR39 Pass A preserves continuity without collapsing different life structures into one subject matter", () => {
  assert.match(GENESIS_REPLACEMENT_PASS_A_PROMPT, /priorEpisodes as continuity and anti-repetition context/iu);
  assert.match(GENESIS_REPLACEMENT_PASS_A_PROMPT, /do not repeatedly default to the same subject matter/iu);
  assert.match(GENESIS_REPLACEMENT_PASS_A_PROMPT, /underused domain afforded by this World and place/iu);
  assert.match(GENESIS_REPLACEMENT_PASS_A_PROMPT, /Do not invent a domain solely for diversity/iu);
});
