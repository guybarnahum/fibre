...
function record(event, revision = 1, overrides = {}) {
  const subject = {
    originEventRef: event.event_id,
    slot: "seed-perspective",
  };
  return {
...
test("autobiographical memory reinterprets perspective without rewriting history or earning standing", () =>
  withDatabase((path) => {
    ...
    const db = new DatabaseSync(path);
    const before = db.prepare("SELECT payload_json FROM thread_events WHERE event_id=?")
      .get(event.event_id).payload_json;
    db.close();
    ...
    assert.deepEqual(store.inspectThread(fixture.threadId).standingCredit, {
    ...
    const after = check.prepare("SELECT payload_json FROM thread_events WHERE event_id=?")
      .get(event.event_id).payload_json;
    check.close();
...
test("memory evidence classes remain explicit and prior epistemic evidence cannot disappear", () =>
  withDatabase((path) => {
    ...
    const store = openAutobiographicalMemoryStore(path);
    assert.throws(
      () => store.recordMemory(record(event, 1, { eventRefs: ["evt_not_real"] })),
      TypeError,
    );
    assert.throws(
      () => store.recordMemory(record(event, 1, { supportingEvidenceRefs: ["evt_not_real"] })),
      AutobiographicalMemoryConflictError,
    );
    ...
    store.recordMemory(record(event, 2, {
      supportingEvidenceRefs: [],
      contradictingEvidenceRefs: [event.event_id],
    }));
    store.close();
  }));
...
test("memory subject identity and subject history cannot be swapped during reinterpretation", () =>
  withDatabase((path) => {
    ...
    assert.throws(
      () => store.recordMemory(record(event, 2, {
        subject: { originEventRef: event.event_id, slot: "different-memory" },
      })),
      TypeError,
    );
    assert.throws(
      () => store.recordMemory(record(event, 2, { eventRefs: ["evt_not_real"] })),
      TypeError,
    );
    store.close();
  }));
...
test("subjectPeriod must actually contain the history events the memory is about", () =>
  withDatabase((path) => {
    ...
    const store = openAutobiographicalMemoryStore(path);
    assert.throws(
      () => store.recordMemory(record(event, 1, {
        subjectPeriod: {
          startAt: "1998-01-01T00:00:00Z",
...
test("memory visibility cannot widen without disclosure authority", () =>
  withDatabase((path) => {
    ...
    store.recordMemory(record(event));
    assert.throws(
      () => store.recordMemory(record(event, 2, { visibility: "public" })),
      AutobiographicalMemoryConflictError,
    );
    store.close();
  }));
...
test("memory schema refuses caller-written recall events and self-authored Development labels", () => {
  ...
  assert.throws(() => normalizeAutobiographicalMemory({
    ...candidate,
    rememberedAt: candidate.recordedAt,
  }));
  assert.throws(() => normalizeAutobiographicalMemory({
    ...candidate,
    lastRecalledAt: candidate.recordedAt,
  }));
  assert.throws(() => normalizeAutobiographicalMemory({
    ...candidate,
    authorship: { ...candidate.authorship, kind: "thread_self_authored" },
  }));
});
