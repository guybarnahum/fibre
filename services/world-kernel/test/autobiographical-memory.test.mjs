...
test("autobiographical memory reinterprets perspective without rewriting history or earning standing", () =>
  withDatabase((path) => {
    ...
    db.close();
    const store = openAutobiographicalMemoryStore(localWorldStateStorage(path));
...
test("memory survives restart and read-only inspection cannot author", () =>
  withDatabase((path) => {
    const event = seed(path);
    const writer = openAutobiographicalMemoryStore(localWorldStateStorage(path));
    writer.recordMemory(record(event));
    writer.close();
    const reader = openAutobiographicalMemoryInspectionStore(localWorldStateStorage(path));
