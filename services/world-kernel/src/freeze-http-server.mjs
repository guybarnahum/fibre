...
function problem(error) {
  if (error instanceof FreezeNotFoundError) return [404, "FREEZE_NOT_FOUND", error.message, {}];
  if (error instanceof FreezeConflictError) return [409, "FREEZE_CONFLICT", error.message, {}];
  if (error instanceof AuthorizationConsumedError) {
    return [409, "AUTHORIZATION_CONSUMED", error.message, {}];
  }
  if (error instanceof FreezeStateChangedError) {
    return [409, "FREEZE_STATE_CHANGED", error.message, {}];
  }
  if (error instanceof FreezeRejectedError) return [422, "FREEZE_REJECTED", error.message, {}];
...
export function createFreezeWorldKernelHttpServer({
  service,
  privateToken = null,
  ...
  server.on("request", async (request, response) => {
    ...
    try {
      ...
      if (typeof service.freezeRuntime !== "function") {
        const error = new TypeError("M1 freeze is not enabled");
        error.httpStatus = 503;
        error.httpCode = "FREEZE_DISABLED";
        throw error;
      }
      if (route.integrity) {
        ...
        return writeJson(
          response,
          200,
          service.verifyFreezeIntegrity(route.threadId, route.sessionId),
          id,
        );
      }
      if (request.method === "GET") {
        return writeJson(
          response,
          200,
          { freeze: service.getFreezeReport(route.threadId, route.sessionId) },
          id,
        );
      }
      ...
      const result = service.freezeRuntime(route.threadId, route.sessionId, body);
