...
function normalizeTimeFrame(candidate) {
  ...
  if (Date.parse(candidate.endAt) < Date.parse(candidate.startAt)) {
    throw new TypeError("passB.world.timeFrame ends before it starts");
  }
  return structuredClone(candidate);
}

function normalizePlace(candidate, index) {
  assertPlainObject(`passB.world.places[${index}]`, candidate);
  assertExactKeys(`passB.world.places[${index}]`, candidate, ["placeId", "description"]);
  assertId(`passB.world.places[${index}].placeId`, candidate.placeId);
  assertNonEmpty(`passB.world.places[${index}].description`, candidate.description);
  return structuredClone(candidate);
}
...
export function normalizePassBWorld(candidate) {
  ...
  if (!Array.isArray(candidate.places) || candidate.places.length === 0) {
    throw new TypeError("passB.world.places must be a non-empty array");
  }
  ...
  for (const key of ["familyRelations", "languages", "availableInstitutions", "affordedRoles"]) {
    assertStringArray(`passB.world.${key}`, candidate[key]);
  }
...
export function normalizePassBHistoryEpisode(candidate, index = 0) {
  ...
  if (!Array.isArray(candidate.introducedParticipants)) {
    throw new TypeError(`${path}.introducedParticipants must be an array`);
  }
...
function normalizeUncertainty(candidate, path) {
  ...
  if (candidate.length > GENESIS_PASS_B_POLICY.maxUncertaintyItems) {
    throw new TypeError(`${path} exceeds ${GENESIS_PASS_B_POLICY.maxUncertaintyItems} items`);
  }
  return candidate.map((item, index) => {
    assertNonEmpty(`${path}[${index}]`, item);
    if (utf8Bytes(item) > GENESIS_PASS_B_POLICY.maxUncertaintyItemBytes) {
      throw new TypeError(`${path}[${index}] exceeds ${GENESIS_PASS_B_POLICY.maxUncertaintyItemBytes} UTF-8 bytes`);
    }
    return item;
  });
}
...
function normalizePriorMemory(candidate, index) {
  ...
  if (utf8Bytes(candidate.rememberedContent) > GENESIS_PASS_B_POLICY.maxRememberedContentBytes) {
    throw new TypeError(`${path}.rememberedContent exceeds ${GENESIS_PASS_B_POLICY.maxRememberedContentBytes} UTF-8 bytes`);
  }
  if (!PASS_B_FORMATION_MODES.includes(candidate.formationMode)) {
    throw new TypeError(`${path}.formationMode is invalid`);
  }
  return structuredClone({
    ...candidate,
    episodeRefs: [...candidate.episodeRefs],
    uncertainty: normalizeUncertainty(candidate.uncertainty, `${path}.uncertainty`),
  });
}

function normalizeAssignment(candidate) {
  assertPlainObject("passB.assignment", candidate);
  assertExactKeys("passB.assignment", candidate, [
    "formationMode",
    "priorTreatmentMemoryExposure",
    "analysisStratum",
  ]);
  if (!PASS_B_FORMATION_MODES.includes(candidate.formationMode)) {
    throw new TypeError("passB.assignment.formationMode is invalid");
  }
  assertBoolean("passB.assignment.priorTreatmentMemoryExposure", candidate.priorTreatmentMemoryExposure);
  if (!PASS_B_ANALYSIS_STRATA.includes(candidate.analysisStratum)) {
    throw new TypeError("passB.assignment.analysisStratum is invalid");
  }
  const expectedStratum = candidate.formationMode === "life_plus_genome"
    ? "life_plus_genome"
    : candidate.priorTreatmentMemoryExposure
      ? "life_only_exposed"
      : "life_only_unexposed";
  if (candidate.analysisStratum !== expectedStratum) {
    throw new TypeError(`passB.assignment.analysisStratum must be ${expectedStratum}`);
  }
  return structuredClone(candidate);
}
...
function normalizeExposurePolicy(candidate) {
  ...
  if (!PASS_B_GENOME_EXPOSURE_KINDS.includes(candidate.kind)) {
    throw new TypeError("passB.genomeExposure.policy.kind is invalid");
  }
  if (candidate.kind === "whole_genome") {
    if (candidate.k !== null) throw new TypeError("whole_genome exposure policy must use k=null");
  } else {
    assertFiniteNumber("passB.genomeExposure.policy.k", candidate.k, { integer: true, minimum: 1 });
  }
  return structuredClone(candidate);
}
...
export function normalizePassBGenomeExposure(candidate) {
  ...
  assertExactKeys("passB.genomeExposure", candidate, [
    "policy",
    "genomeRef",
    "genomeDigest",
    "totalLoci",
  ...
  if (!/^sha256:[0-9a-f]{64}$/.test(candidate.genomeDigest)) {
    throw new TypeError("passB.genomeExposure.genomeDigest must be a SHA-256 digest");
  }
  assertFiniteNumber("passB.genomeExposure.totalLoci", candidate.totalLoci, { integer: true, minimum: 2 });
  if (!Array.isArray(candidate.loci) || candidate.loci.length === 0) {
    throw new TypeError("passB.genomeExposure.loci must be a non-empty array");
  }
  const loci = candidate.loci.map(normalizeExposedLocus).sort((a, b) => a.ordinal - b.ordinal);
  loci.forEach((locus, index) => {
    if (locus.ordinal !== index + 1) {
      throw new TypeError("Pass-B genome exposure must be ordinal-prefix deterministic, never relevance-selected");
    }
  });
  const expectedCount = policy.kind === "whole_genome" ? candidate.totalLoci : policy.k;
  if (loci.length !== expectedCount) {
    throw new TypeError(`Pass-B genome exposure must contain exactly ${expectedCount} loci under its frozen policy`);
  }
...
function normalizePolicyWitness(candidate) {
  ...
  if (candidate.policyVersion !== GENESIS_PASS_B_POLICY.version) {
    throw new TypeError("passB.policyWitness.policyVersion is not supported");
  }
...
export function normalizePassBInput(candidate) {
  ...
  if (candidate.inputVersion !== GENESIS_PASS_B_INPUT_VERSION) {
    throw new TypeError("passB.inputVersion is not supported");
  }
  ...
  if (Date.parse(candidate.rememberingAt) > Date.parse(candidate.chronologyEndsAt)) {
    throw new TypeError("Pass-B remembering moment exceeds chronologyEndsAt");
  }
  if (!Array.isArray(candidate.history) || candidate.history.length === 0) {
    throw new TypeError("passB.history must be a non-empty array");
  }
  ...
  for (const episode of history) {
    ...
    if (occurredAt > Date.parse(candidate.rememberingAt)) {
      throw new TypeError("Pass B cannot see history after the remembering moment");
    }
  ...
  if (assignment.priorTreatmentMemoryExposure !== observedPriorTreatment) {
    throw new TypeError("passB.assignment.priorTreatmentMemoryExposure does not match visible prior remembered memory history");
  }
  const genomeExposure = candidate.genomeExposure === null ? null : normalizePassBGenomeExposure(candidate.genomeExposure);
  if (assignment.formationMode === "life_only" && genomeExposure !== null) {
    throw new TypeError("life_only Pass-B cognition must not receive a genome exposure");
  }
  if (assignment.formationMode === "life_plus_genome" && genomeExposure === null) {
    throw new TypeError("life_plus_genome Pass-B cognition requires the frozen genome exposure");
  }
  const policyWitness = normalizePolicyWitness(candidate.policyWitness);
  if (assignment.formationMode === "life_only" && policyWitness.genomeExposurePolicyRef !== null) {
    throw new TypeError("life_only Pass-B call must not carry a genome exposure policy ref");
  }
  if (assignment.formationMode === "life_plus_genome" && policyWitness.genomeExposurePolicyRef === null) {
    throw new TypeError("life_plus_genome Pass-B call requires a genome exposure policy ref");
  }
  return structuredClone({
    ...candidate,
    world,
...
function normalizeRememberedContent(value) {
  ...
  if (utf8Bytes(value) > GENESIS_PASS_B_POLICY.maxRememberedContentBytes) {
    throw new TypeError(`MemoryFormation.rememberedContent exceeds ${GENESIS_PASS_B_POLICY.maxRememberedContentBytes} UTF-8 bytes`);
  }
  return value;
}
...
export function normalizePassBModelOutput(candidate, inputCandidate) {
  ...
  if (!["remembered", "not_remembered"].includes(candidate.outcome)) {
    throw new TypeError("Pass-B model output outcome is invalid");
  }
  assertStringArray("Pass-B model output.episodeRefs", candidate.episodeRefs);
  const uncertainty = normalizeUncertainty(candidate.uncertainty, "Pass-B model output.uncertainty");
  const historyRefs = new Set(input.history.map((episode) => episode.episodeId));
  for (const ref of candidate.episodeRefs) {
    if (!historyRefs.has(ref)) throw new TypeError(`Pass-B model output episodeRef ${ref} is not visible history`);
  }
...
export function passBInputDigest(candidate) {
  return digest(normalizePassBInput(candidate));
}

export function passBMemoryFormationDigest(candidate) {
