...
} from "./genesis-replacement-pass-b.mjs";
import { normalizeGenesisWorldSpec } from "./genesis-domain.mjs";

export const GENESIS_REPLACEMENT_GENOME_EXPOSURE_POLICY = Object.freeze({
...
function genomeExposure(genome) {
  ...
  if (genome.loci.length !== GENESIS_REPLACEMENT_GENOME_EXPOSURE_POLICY.locusCount) {
  ...
  return {
    policy: {
      kind: GENESIS_REPLACEMENT_GENOME_EXPOSURE_POLICY.kind,
      k: GENESIS_REPLACEMENT_GENOME_EXPOSURE_POLICY.k,
    },
...
export function buildReplacementPassBInput({
  threadId,
  bornAt,
  worldSpec,
  episodes,
  ...
  if (!Number.isInteger(callOrdinal) || callOrdinal < 1 || callOrdinal > GENESIS_REPLACEMENT_PASS_B_HORIZONS.length) {
  ...
  const horizon = GENESIS_REPLACEMENT_PASS_B_HORIZONS[index];
  const formationMode = GENESIS_REPLACEMENT_PASS_B_FORMATION_MODES[index];
  ...
  return normalizePassBInput({
    ...
    policyWitness: {
      ...
      genomeExposurePolicyRef: formationMode === "life_plus_genome"
        ? GENESIS_REPLACEMENT_GENOME_EXPOSURE_POLICY.policyVersion
        : null,
    },
  });
}
