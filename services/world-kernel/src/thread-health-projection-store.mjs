...
export function createThreadHealthReader({
  projectionStore,
  presentationReader,
  diagnosisService,
  ...
  if (!projectionStore
    || typeof projectionStore.worldWitness !== "function"
    || typeof projectionStore.get !== "function"
    || typeof projectionStore.put !== "function") {
    throw new TypeError("Thread health reader requires a projection store");
  }
  if (!presentationReader || typeof presentationReader.getSnapshotDigest !== "function") {
    throw new TypeError("Thread health reader requires Presentation snapshot digest");
  }
  if (!diagnosisService || typeof diagnosisService.diagnose !== "function") {
    throw new TypeError("Thread health reader requires diagnose()");
  }

  return Object.freeze({
    async diagnose(threadId) {
      const world = projectionStore.worldWitness(threadId);
      if (world === null) return diagnosisService.diagnose(threadId);

      const witness = Object.freeze({
        ...world.diagnosis,
        presentationSnapshotDigest:await presentationReader.getSnapshotDigest(threadId),
      });
      const cached = projectionStore.get(threadId, witness);
      if (cached !== null) return cached;

      const diagnosis = await diagnosisService.diagnose(threadId);
      return projectionStore.put(threadId, witness, diagnosis, { updatedAt:now() });
    },
  });
}
