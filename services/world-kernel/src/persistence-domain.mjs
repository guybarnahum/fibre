...
export function validateThreadSnapshot(thread) {
  ...
  if (thread.accounts !== undefined) {
    ...
    assertFiniteNumber("thread.accounts.fibreCredits", thread.accounts.fibreCredits, {
      integer: true,
      minimum: 0,
    });
    assertFiniteNumber("thread.accounts.usdAvailable", thread.accounts.usdAvailable, {
      minimum: 0,
    });
    assertFiniteNumber(
      "thread.accounts.modelTokensAvailable",
      thread.accounts.modelTokensAvailable,
  ...
  if (thread.provenance.lastEventId !== undefined) {
    assertId("thread.provenance.lastEventId", thread.provenance.lastEventId);
  }
...
export function validateStoredThread(threadId, thread) {
  try {
    ...
    throw new IntegrityError(`Stored Thread ${threadId} is invalid: ${error.message}`);
  }
}
...
export function validateCommand(command) {
  ...
  assertFiniteNumber("command.expectedVersion", command.expectedVersion, {
    integer: true,
    minimum: 1,
  });
  ...
  if (payloadBytes > MAX_COMMAND_PAYLOAD_BYTES) {
    throw new TypeError(
      `command.payload exceeds ${MAX_COMMAND_PAYLOAD_BYTES} UTF-8 bytes`,
    );
  }
...
export function eventIdForCommand(command, digest) {
  return `evt_${command.threadId}_${digest.slice(7, 31)}`;
}
...
export function normalizeSeedSnapshot(thread) {
  ...
  normalized.provenance = {
    ...normalized.provenance,
    lastEventId: seedEventId(normalized),
  };
...
export function assertCommandLifecycle(thread, command, ErrorType = LifecycleCommandError) {
  if (command.type === "UPDATE_SELF_MODEL" && !UPDATE_SELF_MODEL_STATUSES.has(thread.status)) {
    throw new ErrorType(
      `UPDATE_SELF_MODEL cannot act on Thread ${thread.threadId} while status is ${thread.status}`,
    );
  }
}
...
export function rowToEvent(row) {
  try {
    const event = {
      ...
      provenance: parseJson(`event ${row.event_id} provenance`, row.provenance_json),
    };
    ...
    if (!EVENT_TYPES.has(event.eventType)) {
      throw new TypeError(`unsupported type ${event.eventType}`);
    }
    if (!Number.isSafeInteger(event.sequence) || event.sequence < 1) {
      throw new TypeError("invalid sequence");
    }
    if (!Number.isSafeInteger(event.expectedVersion) || event.expectedVersion < 0) {
      throw new TypeError("invalid expected version");
    }
    if (!Number.isSafeInteger(event.resultingVersion) || event.resultingVersion < 1) {
      throw new TypeError("invalid resulting version");
    }
    if (!Number.isSafeInteger(event.payloadSchemaVersion) || event.payloadSchemaVersion < 1) {
      throw new TypeError("invalid payload schema version");
    }
    if (event.commandId !== null) assertId(`event ${event.eventId} commandId`, event.commandId);
    if (event.commandDigest !== null && !/^sha256:[0-9a-f]{64}$/.test(event.commandDigest)) {
      throw new TypeError("invalid command digest");
    }
...
export function applyEventToThread(thread, event) {
  if (event.eventType === "THREAD_SEEDED") {
    ...
    if (snapshot.threadId !== event.threadId) {
      throw new IntegrityError(
        `seed event ${event.eventId} snapshot belongs to ${snapshot.threadId}, not ${event.threadId}`,
      );
    }
    if (snapshot.provenance.lastEventId !== event.eventId) {
      throw new IntegrityError(`seed event ${event.eventId} is not the snapshot last event`);
    }
    ...
    if (event.commandId !== null || event.commandDigest !== null) {
      throw new IntegrityError(`seed event ${event.eventId} must not carry command metadata`);
    }
    return snapshot;
  }
  ...
  if (event.eventType === "SELF_MODEL_UPDATED") {
    ...
    if (event.threadId !== thread.threadId) {
      throw new IntegrityError(`event ${event.eventId} belongs to another Thread`);
    }
    ...
    try {
      ...
      throw new IntegrityError(`event ${event.eventId} command is invalid: ${error.message}`);
    }
    const digest = commandDigest(command);
    if (event.commandDigest !== digest) {
      throw new IntegrityError(`event ${event.eventId} command digest does not match content`);
    }
    if (eventIdForCommand(command, digest) !== event.eventId) {
      throw new IntegrityError(`event ${event.eventId} does not match its command digest`);
    }
    let replayed;
    try {
      replayed = applyCommandToThread(thread, command, event.eventId, IntegrityError);
    } catch (error) {
      if (error instanceof IntegrityError) throw error;
      throw new IntegrityError(`event ${event.eventId} cannot be applied: ${error.message}`);
    }
