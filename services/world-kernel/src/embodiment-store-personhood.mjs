  ...
  history(threadId, embodimentId, { required = true } = {}) {
    ...
    for (let i = 0; i < records.length; i += 1) {
      ...
      if (i > 0) {
        ...
        if (record.supersedesRevision !== prior.revision) throw new IntegrityError(`embodiment ${embodimentId} does not supersede its immediate predecessor`);
        if (Date.parse(record.recordedAt) < Date.parse(prior.recordedAt)) throw new IntegrityError(`embodiment ${embodimentId} recordedAt moves backwards`);
        ...
        if (record.representationKind === "human_source_derivative") {
          if (!isSuperset(record.sourceReferences, prior.sourceReferences)) throw new IntegrityError("human-derived embodiment discarded prior source evidence");
          if (!isSuperset(record.permissionReferences, prior.permissionReferences)) throw new IntegrityError("human-derived embodiment discarded prior permission authority");
        }
      }
  ...
  record(candidate) {
    ...
    try {
      ...
      if (record.representationKind === "human_source_derivative") {
        ...
        if (sourcePartyIds.size !== 1) throw new EmbodimentConflictError("human-derived embodiment must resolve to one human source identity");
      }
      ...
      if (history.length > 0) {
        ...
        if (VISIBILITY_RANK[record.visibility] > VISIBILITY_RANK[prior.visibility]) {
          const newAuthorityRefs = record.permissionReferences.filter((reference) => !prior.permissionReferences.includes(reference));
          if (record.representationKind === "human_source_derivative") {
            if (newAuthorityRefs.length === 0) throw new EmbodimentConflictError("widening human-derived embodiment visibility requires new rights authority");
