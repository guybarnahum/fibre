...
export function assertHistoricalEnvelopeRealized(episode, envelope) {
  if (episode.occurredAt !== envelope.occurredAt) invariant("GENESIS_HISTORICAL_ENVELOPE_CONFLICT", `episode ${episode.episodeId} changed frozen historical-envelope occurredAt`);
  if (episode.placeRef !== envelope.placeRef) invariant("GENESIS_HISTORICAL_ENVELOPE_CONFLICT", `episode ${episode.episodeId} changed frozen historical-envelope place`);
  const expectedStructure = envelope.selectionKind === "world_emergent" ? null : envelope.structureRef;
  if (episode.structureRef !== expectedStructure) invariant("GENESIS_HISTORICAL_ENVELOPE_CONFLICT", `episode ${episode.episodeId} changed frozen historical-envelope structure`);
  if (envelope.counterpart !== null && !episode.participantRefs.includes(envelope.counterpart.participantId)) invariant("GENESIS_HISTORICAL_ENVELOPE_CONFLICT", `episode ${episode.episodeId} omitted frozen historical-envelope counterpart`);
  ...
  if (weekdays.some((weekday) => weekday !== envelope.localWeekday)) invariant("GENESIS_EPISODE_TIME_CONFLICT", `episode ${episode.episodeId} narrates a weekday inconsistent with local civil time`);
  const allowed = allowedNarrativeDayparts(envelope.daypart);
  const dayparts = mentionedDayparts(episode.observableAction);
  if (dayparts.some((part) => !allowed.has(part))) invariant("GENESIS_EPISODE_TIME_CONFLICT", `episode ${episode.episodeId} narrates a daypart inconsistent with local civil time`);
  return episode;
}
