...
export function normalizePresentationAssetSlot(value) {
  ...
  if (!PRESENTATION_ASSET_ENTITY_KINDS.includes(value.entityKind)) {
  ...
  if (!PRESENTATION_ASSET_SLOT_STATUSES.includes(value.status)) {
  ...
  if (value.status === "missing" && brief === null) {
    throw new TypeError(`${name} missing slot requires a generation brief`);
  }
  if (value.status === "deferred" && deferredReason === null) {
    throw new TypeError(`${name} deferred slot requires deferredReason`);
  }
  if (value.status !== "deferred" && deferredReason !== null) {
    throw new TypeError(`${name}.deferredReason is only valid for deferred slots`);
  }
...
export function presentationAssetIdentityDigest(slotValue, {
  providerProfile,
  ...
  // Presentation owns semantic sameness. Operational witness fields such as
  // requestedAt and projection snapshot pointers deliberately do not participate.
  return presentationAssetSourceDigest({
...
export function normalizePresentationAssetDemand(value) {
  ...
  assertPlainObject(name, value);
  if (value.demandVersion !== PRESENTATION_ASSET_DEMAND_VERSION) {
    throw new TypeError(`${name}.demandVersion is unsupported`);
  }
  assertNonEmpty(`${name}.demandId`, value.demandId);
  assertNonEmpty(`${name}.slotKey`, value.slotKey);
  const identityDigest = digest(`${name}.identityDigest`, value.identityDigest);
  const sourceDigest = digest(`${name}.sourceDigest`, value.sourceDigest);
  ...
  const supersedesDemandId = nullableNonEmpty(
    `${name}.supersedesDemandId`,
    value.supersedesDemandId ?? null,
  );
  const job = normalizeAssetGenerationJob(value.job);
  const expectedJobId = `assetjob_${identityDigest.slice("sha256:".length)}`;
  if (job.jobId !== expectedJobId) throw new TypeError(`${name}.job does not match identityDigest`);
  if (job.providerProfile !== value.providerProfile) {
    throw new TypeError(`${name}.job provider profile does not match demand`);
  }
  return Object.freeze({
...
function currentDemandBySlot(existingDemands) {
  ...
  for (const raw of existingDemands) {
    ...
    if (bySlot.has(demand.slotKey)) {
      throw new TypeError(`multiple current presentation asset demands for slot ${demand.slotKey}`);
    }
...
export function reconcilePresentationAssets({
  ...
  for (const slot of normalizedSlots) {
    ...
    if (slot.status !== "missing") {
      if (slot.status === "deferred" && current !== null) {
        obsoleteDemands.push(demandPatch(current, "obsolete", {
          reason: "slot_no_longer_eligible",
        }));
      }
      continue;
    }

    const identityDigest = presentationAssetIdentityDigest(slot, {
      providerProfile,
      regenerationKey,
    });
    if (current !== null && current.identityDigest === identityDigest) {
    ...
    const demand = normalizePresentationAssetDemand({
      demandVersion: PRESENTATION_ASSET_DEMAND_VERSION,
      demandId: `presassetdemand_${identityDigest.slice("sha256:".length)}`,
      slotKey: slot.slotKey,
      identityDigest,
      sourceDigest: slot.sourceDigest,
      providerProfile,
    ...
    if (current !== null) {
      supersededDemands.push(demandPatch(current, "superseded", {
        supersededByDemandId: demand.demandId,
      }));
    }
  }
  ...
  for (const [slotKey, current] of currentBySlot) {
    if (!slotKeys.has(slotKey)) {
      obsoleteDemands.push(demandPatch(current, "obsolete", {
        reason: "slot_removed_from_projection",
      }));
    }
  }
