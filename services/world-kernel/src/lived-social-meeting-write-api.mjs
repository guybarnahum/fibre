...
export function createSocialMeetingWriteApi({
  meetingService,
  privateToken,
  now = () => new Date().toISOString(),
}) {
  if (meetingService === null || typeof meetingService !== "object" || typeof meetingService.meet !== "function") {
    throw new TypeError("social meeting API requires meetingService.meet");
  }
  if (typeof privateToken !== "string" || privateToken.trim() === "") {
    throw new TypeError("social meeting API requires privateToken");
  }

  return Object.freeze({
    async fetch(request) {
      const url = new URL(request.url);
      if (url.pathname !== "/internal/social-meeting") return null;
