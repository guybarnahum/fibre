...
Birth Center also keeps a durable disposition for interrupted development attempts. Pending inspection itself is local and returns immediately. Seeing stale work may asynchronously kick one coalesced reconciliation pass, while the scheduler remains the retry mechanism. Reconciliation asks World for the bounded stale Thread set in one query rather than one request per birth; repeated polling cannot duplicate a running pass and retries are throttled. If World contains the Thread, the attempt settles `born`. If Genesis recorded an explicit terminal/non-retryable pre-admission failure and World confirms absence, it settles `stillborn`. Retryable or ambiguous failures remain recoverable/unresolved rather than being guessed, and age alone never decides the outcome.
