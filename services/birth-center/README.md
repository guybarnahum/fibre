...
Birth Center also keeps a durable disposition for interrupted development attempts. Pending inspection is deliberately local and read-only: it never calls World and never performs settlement. The scheduler derives its next wake from local timestamps; on wake, only stale unresolved births are reconciled against World, with one World lookup reused when modern and development records refer to the same Thread. If World already contains the Thread, the attempt settles as `born`; a recorded pre-admission Pass-A validation failure may settle as `stillborn` only when World explicitly confirms absence. World-unavailable or otherwise ambiguous cases remain unresolved rather than being guessed.
