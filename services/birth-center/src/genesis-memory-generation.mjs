...
export function assertGenesisLifePassBSchedule({
  ...
  if (canonicalJson(horizons) !== canonicalJson(GENESIS_LIFE_PASS_B_HORIZONS)) throw new TypeError("replacement Pass-B history horizons drift");
  if (canonicalJson(formationModes) !== canonicalJson(GENESIS_LIFE_PASS_B_FORMATION_MODES)) throw new TypeError("replacement Pass-B formation modes drift");
  if (historyLength !== GENESIS_LIFE_PASS_B_HORIZONS.at(-1)) throw new TypeError("replacement Pass-B final horizon must consume the complete sparse history");
  return true;
}
...
export async function generateGenesisPassBMemory({ adapter, input, clientRequestId } = {}) {
  ...
  const invoke = async ({ prompt, kind, generatedVersion }) => {
    if (!prompt.includes(GENESIS_LIFE_SPARSE_HISTORY_NOTICE)) throw new TypeError("replacement Pass-B prompt omitted sparse-history authority");
