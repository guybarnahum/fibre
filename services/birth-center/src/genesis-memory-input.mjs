...
function genomeExposure(genome) {
  if (genome === null || typeof genome !== "object" || !Array.isArray(genome.loci)) throw new TypeError("replacement Pass-B treatment requires a symbolic genome");
  if (genome.loci.length !== GENESIS_LIFE_GENOME_EXPOSURE_POLICY.locusCount) throw new TypeError("replacement Pass-B genome locus count drift");
  const loci = [...genome.loci].sort((a, b) => a.ordinal - b.ordinal).map(({ locusId, ordinal, value }) => ({ locusId, ordinal, value }));
  if (loci.some((locus, index) => locus.ordinal !== index + 1)) throw new TypeError("replacement Pass-B genome loci are not in canonical ordinal order");
  return {
    policy: { kind: GENESIS_LIFE_GENOME_EXPOSURE_POLICY.kind, k: GENESIS_LIFE_GENOME_EXPOSURE_POLICY.k },
...
export function buildGenesisPassBInput({ threadId, bornAt, worldSpec, episodes, windows, callOrdinal, priorRememberedMemories, genome } = {}) {
  assertGenesisLifePassBSchedule({
    horizons: GENESIS_LIFE_PASS_B_HORIZONS,
    formationModes: GENESIS_LIFE_PASS_B_FORMATION_MODES,
    historyLength: episodes.length,
  });
  if (!Number.isInteger(callOrdinal) || callOrdinal < 1 || callOrdinal > GENESIS_LIFE_PASS_B_HORIZONS.length) throw new TypeError("replacement Pass-B callOrdinal is invalid");
  if (!Array.isArray(windows) || windows.length !== episodes.length) throw new TypeError("replacement Pass-B windows must align with admitted history");
  ...
  return normalizePassBInput({
    ...
    policyWitness: {
      ...
      genomeExposurePolicyRef: formationMode === "life_plus_genome" ? GENESIS_LIFE_GENOME_EXPOSURE_POLICY.policyVersion : null,
    },
  });
}
