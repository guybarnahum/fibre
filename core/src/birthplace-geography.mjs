...
export const FIBRE_BIRTHPLACE_GEOGRAPHY = Object.freeze(
  Object.entries(BIRTHPLACE_COORDINATES).map(([place, coordinates]) => entry(place, coordinates)),
);
...
for (const [alias, place] of [
  ...
  const resolved = FIBRE_BIRTHPLACE_GEOGRAPHY.find((candidate) => candidate.place === place);
  if (resolved) ALIASES.set(normalized(alias), Object.freeze({ ...resolved, displayName:alias }));
}

export function resolveBirthplaceGeography(value) {
  return ALIASES.get(normalized(value)) ?? null;
}

export function birthplaceCoordinates(place) {
  const resolved = resolveBirthplaceGeography(place);
