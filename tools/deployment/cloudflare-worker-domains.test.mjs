...
test("Workers Domain API attaches missing custom domain, rereads stable ownership, and verifies delegation", async () => {
  const requests = [];
  let attached = false;
  const client = createCloudflareWorkerDomainClient({
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if ((options.method ?? "GET") === "GET") {
        return response({ success:true, errors:[], result:attached ? [domainResult()] : [] });
      }
      attached = true;
  ...
  assert.match(requests[2].url, /workers\/domains\?hostname=api\.staging\.insidefibre\.com/u);
  assert.match(requests[3].url, /^https:\/\/dns\.google\/resolve\?.*name=insidefibre\.com.*type=NS/u);
});
...
test("Workers Domain convergence fails if attachment does not survive its immediate reread", async () => {
  const client = {
    async listDomains() { return []; },
    async attachDomain() { return domainResult(); },
    async assertPublicDelegation() { throw new Error("must not verify unstable domain"); },
  };
  await assert.rejects(ensureCloudflareWorkerDomain({
    client,
    hostname:"api.staging.insidefibre.com",
    service:"fibre-thread-presentation-staging",
  }), /was not stable after deployment/u);
});
...
test("runtime deployment explicitly converges a generated custom domain after Worker upload", async () => {
  ...
  let attached = false;
  const domainClient = {
    async listDomains({ hostname }) { calls.push(`domain:list:${hostname}`); return attached ? [domainResult({ hostname, service:"fibre-thread-presentation-staging" })] : []; },
    async attachDomain({ hostname, service }) { calls.push(`domain:attach:${hostname}:${service}`); attached = true; return domainResult({ hostname, service }); },
    async assertPublicDelegation({ zoneName }) { calls.push(`domain:dns:${zoneName}`); return { zoneName, nameservers:["ada.ns.cloudflare.com"] }; },
  };
  const client = createWranglerDeploymentClient({
    workerDomainClient:domainClient,
    runner:async (args) => { calls.push(`wrangler:${args[0]}`); return { stdout:"Published https://worker.account.workers.dev", stderr:"", exitCode:0 }; },
...
test("app deployment attaches custom domain only for a live deployment, never a dry-run", async () => {
  ...
  let attached = false;
  const domainClient = {
    async listDomains({ hostname }) { calls.push(`domain:list:${hostname}`); return attached ? [domainResult({ hostname, service:"fibre-status-page-staging" })] : []; },
    async attachDomain({ hostname, service }) { calls.push(`domain:attach:${hostname}:${service}`); attached = true; return domainResult({ hostname, service }); },
    async assertPublicDelegation({ zoneName }) { calls.push(`domain:dns:${zoneName}`); return { zoneName, nameservers:["ada.ns.cloudflare.com"] }; },
  };
