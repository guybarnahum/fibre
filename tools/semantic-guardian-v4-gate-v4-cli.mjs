...
export const SEMANTIC_GUARDIAN_V4_GATE_V4_ARTIFACT = resolve(`artifacts/test-results/${SET.id}.evidence.json`);
export const SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL = resolve(`artifacts/test-results/${SET.id}.judgments.ndjson`);
...
function parseArgs(argv) {
  const options = { summary: false, json: false, summaryOnly: false, evidencePath: SEMANTIC_GUARDIAN_V4_GATE_V4_ARTIFACT, help: false };
  ...
  if (!options.summaryOnly && options.evidencePath !== SEMANTIC_GUARDIAN_V4_GATE_V4_ARTIFACT) throw new Error("--evidence is only valid with --summary-only");
  return options;
}
...
function readJournal(path) {
  ...
  return text.split("\n").map((line, index) => {
    ...
    catch (error) { throw new Error(`standing-gate v4 journal line ${index + 1} is invalid JSON: ${error.message}`); }
  });
}

function readBundle(path) {
  if (!existsSync(path)) throw new Error(`standing-gate v4 evidence artifact not found: ${path}`);
...
function startProgress(path) {
  ...
  const render = () => {
    ...
    let detail = `Guardian ${snapshot.responses}/${EXPECTED_CASES} model responses`;
    if (event?.type === "model_attempt") detail = `Guardian case ${currentCase}/${EXPECTED_CASES} · attempt ${event.attempt}/${event.maximumAttempts} · waiting for ${FROZEN.provider}`;
    else if (event?.type === "operational_failure" && event.retrying) detail = `Guardian case ${currentCase}/${EXPECTED_CASES} · retrying after ${event.failure?.code ?? "provider error"}`;
    process.stderr.write(`\r\u001b[2K${frames[frame % frames.length]} ${detail} · ${elapsedLabel(Date.now() - startedAt)}`);
    frame += 1;
  };
  render();
  ...
  return {
    stop(status, finalSnapshot = null) {
      ...
      process.stderr.write(`\r\u001b[2K${symbol} Standing gate v4 · ${snapshot.responses}/${EXPECTED_CASES} model responses · ${status} · ${elapsedLabel(Date.now() - startedAt)}\n\n`);
    },
  };
}
...
function failureLines(title, failures) {
  ...
  return ["", title, "────────────────────────────────────────", ...failures.flatMap((failure) => [`✗ ${failure.caseId ?? "unknown"}${failure.code ? ` · ${failure.code}` : ""}`, `  ${failure.message}`])];
}

export function formatStandingGateV4Summary(bundle) {
  const report = bundle.report;
  const passedCases = Array.isArray(report.cases) ? report.cases.filter((item) => item.status === "passed").length : 0;
  const lines = [
    "Fibre · Semantic Guardian v4 standing gate v4",
  ...
  if (Array.isArray(report.cases) && report.cases.length > 0) {
    ...
    for (const item of report.cases) {
      ...
      lines.push(`${marker} ${item.caseId}`, `  action=${item.output.proposedAction} · fit=${item.output.participationFit}`, `  why: ${compactText(item.output.rationale, 190)}`);
    }
  }
  ...
  lines.push(
    ...
    report.standingDifferentialGatePassed
      ? "Frozen candidate 4 passed its fresh held-out standing gate, including two state-to-state semantic counterfactuals. This sealed artifact may support Semantic Guardian standing credit."
      : "Frozen candidate 4 did not pass this sealed standing gate. Do not tune or rerun this cycle; preserve the artifact and reassess the standing claim from the evidence.",
  );
  return `${lines.join("\n")}\n`;
}
...
export async function runSealedSemanticGuardianV4GateV4({ environment = process.env } = {}) {
  ...
  const observer = (event) => appendFileSync(SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL, `${JSON.stringify(event)}\n`, "utf8");
  let adapter;
  try {
    ...
    if (mismatches.length > 0) {
      rmSync(SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL, { force: true });
      return { report: blockedV4StandingReportV4(`Frozen runtime boundary mismatch: ${mismatches.join("; ")}`), evidenceArtifactPath: null, evidenceBundle: null };
    }
  } catch (error) {
    rmSync(SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL, { force: true });
    if (error?.code === "MODEL_UNAVAILABLE") return { report: blockedV4StandingReportV4(error.message), evidenceArtifactPath: null, evidenceBundle: null };
    throw error;
  }

  const progress = startProgress(SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL);
  try {
    const report = await runSemanticGuardianV4StandingProofV4({ modelAdapter: adapter });
    const journal = readJournal(SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL);
    const finalSnapshot = { responses: journal.filter((entry) => entry.type === "model_response").length, failures: journal.filter((entry) => entry.type === "operational_failure").length };
    const bundle = bundleFor(report, journal);
    writeBundle(SEMANTIC_GUARDIAN_V4_GATE_V4_ARTIFACT, bundle);
    progress.stop(report.status, finalSnapshot);
    rmSync(SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL, { force: true });
    return { report, evidenceArtifactPath: SEMANTIC_GUARDIAN_V4_GATE_V4_ARTIFACT, evidenceBundle: bundle };
  } catch (error) {
    const journal = readJournal(SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL);
    const providerAttempted = journal.some((entry) => entry.type === "model_attempt");
    if (!providerAttempted) {
      progress.stop("blocked", progressSnapshot(SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL));
      rmSync(SEMANTIC_GUARDIAN_V4_GATE_V4_JOURNAL, { force: true });
      return { report: blockedV4StandingReportV4(error?.message ?? String(error)), evidenceArtifactPath: null, evidenceBundle: null };
    }
    const finalSnapshot = { responses: journal.filter((entry) => entry.type === "model_response").length, failures: journal.filter((entry) => entry.type === "operational_failure").length };
    const fatalError = { name: error?.constructor?.name ?? "Error", code: error?.code ?? null, message: error?.message ?? String(error) };
    const failedReport = { ...blockedV4StandingReportV4("The sealed standing gate terminated after a real provider attempt."), status: "failed", casesAttempted: finalSnapshot.responses, casesPlanned: EXPECTED_CASES };
    const bundle = bundleFor(failedReport, journal, fatalError);
    if (!existsSync(SEMANTIC_GUARDIAN_V4_GATE_V4_ARTIFACT)) writeBundle(SEMANTIC_GUARDIAN_V4_GATE_V4_ARTIFACT, bundle);
...
async function main() {
  ...
  const bundle = result.evidenceBundle ?? {
    ...
    judgments: [], operationalAttempts: [], journal: [],
  };
  if (options.summary) process.stdout.write(formatStandingGateV4Summary(bundle));
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  main().catch((error) => {
    process.stderr.write(`Semantic Guardian v4 standing gate v4 failed: ${error?.message ?? String(error)}\n`);
    process.exitCode = 1;
  });
}
