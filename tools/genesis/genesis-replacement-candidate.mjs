...
import { generateReplacementHistoricalEpisode } from "../../services/world-kernel/src/genesis-replacement-pass-a.mjs";
import { buildReplacementPassBInput } from "../../services/world-kernel/src/genesis-replacement-pass-b-input.mjs";
import { generateReplacementPassBMemory } from "../../services/world-kernel/src/genesis-replacement-pass-b.mjs";
import {
  generateReplacementInitialMeaning,
  generateReplacementReinterpretation,
...
export const GENESIS_REPLACEMENT_CANDIDATE_VERSION = "pr39-replacement-v2-candidate-v1";
...
export async function generateReplacementThreadCandidate({
  slotPlan,
  adapter,
  ...
  for (let callOrdinal = 1; callOrdinal <= 6; callOrdinal += 1) {
    const input = buildReplacementPassBInput({
      threadId: slotPlan.threadId,
    ...
    const cResult = await generateReplacementInitialMeaning({
      adapter,
      input: cInput,
  ...
  for (const scheduled of reinterpretationSchedule.filter((item) => item.run)) {
    ...
    const result = await generateReplacementReinterpretation({
      adapter,
      input,
  ...
  const core = {
    candidateVersion: GENESIS_REPLACEMENT_CANDIDATE_VERSION,
    attemptStartedAt,
    slot: slotPlan.slot,
    threadId: slotPlan.threadId,
