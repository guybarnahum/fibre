...
export const REPRO_TOOL_TEST_PATHS = Object.freeze([
  ...
  "tools/repro/pr39-genome/genesis-genome-positive-control.test.mjs",
...
function relativePathForLifecycle(path, root = DEFAULT_TEST_ROOT) {
  if (!resolve(path).startsWith(`${resolve(root)}/`) && !resolve(path).startsWith(`${resolve(root)}\\`)) {
...
export function discoverTestSuites(root = DEFAULT_TEST_ROOT) {
  ...
  const all = [...domain, ...worldKernel, ...tools].sort();
