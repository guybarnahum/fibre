...
export const SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT = resolve(
  `artifacts/test-results/${SET.id}.evidence.json`,
);
export const SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL = resolve(
  `artifacts/test-results/${SET.id}.judgments.ndjson`,
);

const EXPECTED_CASES = buildSemanticGuardianV4StandingCasesV2().length;

function usage() {
  return `Fibre Semantic Guardian v4 standing gate v2\n\nUsage:\n  npm run guardian:gate\n  npm run guardian:gate -- --summary\n  npm run guardian:gate -- --json\n  npm run guardian:gate -- --summary --json\n  npm run guardian:gate -- --summary-only\n  npm run guardian:gate -- --summary-only --evidence <path>\n\nThis is a one-shot sealed standing gate for ${SET.id}.\nMissing credentials/configuration block without consuming the cycle. Once a real provider attempt starts, the cycle seals pass or fail.\n`;
}
...
function parseArgs(argv) {
  const options = {
    ...
    json: false,
    summaryOnly: false,
    evidencePath: SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT,
    help: false,
  };
  ...
  if (!options.summaryOnly && options.evidencePath !== SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT) {
...
function readJournal(path) {
  ...
  return text.split("\n").map((line, index) => {
    ...
    catch (error) { throw new Error(`standing-gate v2 journal line ${index + 1} is invalid JSON: ${error.message}`); }
  });
}

function readBundle(path) {
  if (!existsSync(path)) throw new Error(`standing-gate v2 evidence artifact not found: ${path}`);
...
function startProgress(path) {
  ...
  process.stderr.write(
    `\nFibre · Semantic Guardian v4 standing gate v2\n` +
  ...
  return {
    stop(status) {
      ...
      process.stderr.write(
        `\r\u001b[2K${symbol} Standing gate v2 · ${snapshot.responses}/${EXPECTED_CASES} model responses · ${status} · ${elapsedLabel(Date.now() - startedAt)}\n\n`,
      );
    },
  };
}
...
export function formatStandingGateV2Summary(bundle) {
  ...
  lines.push(
    ...
    report.standingDifferentialGatePassed
      ? "The cognition-equivalent frozen candidate 2 passed its fresh held-out standing gate. This sealed artifact may support Semantic Guardian standing credit."
      : "Frozen candidate 2 did not pass this sealed standing gate. Do not tune or rerun this cycle; any further candidate requires another frozen boundary and another fresh held-out gate.",
  );
  return `${lines.join("\n")}\n`;
}

export async function runSealedSemanticGuardianV4GateV2({ environment = process.env } = {}) {
  assertSemanticGuardianV4FrozenBoundaryV2();
  if (existsSync(SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT) || existsSync(SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL)) {
    return {
      report: blockedV4StandingReportV2(
        `Standing gate ${SET.id} is already sealed by an existing evidence artifact or journal. Never rerun a sealed cycle.`,
      ),
      evidenceArtifactPath: existsSync(SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT) ? SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT : null,
      evidenceBundle: null,
    };
  }

  mkdirSync(dirname(SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL), { recursive: true });
  const observer = (event) => appendFileSync(SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL, `${JSON.stringify(event)}\n`, "utf8");
  let runtime;
  let adapter;
  try {
    runtime = createModelRuntime({ environment, observer });
    ...
    if (mismatches.length > 0) {
      rmSync(SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL, { force: true });
      return {
        report: blockedV4StandingReportV2(`Frozen runtime boundary mismatch: ${mismatches.join("; ")}`),
    ...
    if (error?.code === "MODEL_UNAVAILABLE") {
      return { report: blockedV4StandingReportV2(error.message), evidenceArtifactPath: null, evidenceBundle: null };
    }
    throw error;
  }

  const progress = startProgress(SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL);
  try {
    const report = await runSemanticGuardianV4StandingProofV2({ modelAdapter: adapter });
    const journal = readJournal(SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL);
    const bundle = bundleFor(report, journal);
    writeBundle(SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT, bundle);
    progress.stop(report.status);
    rmSync(SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL, { force: true });
    return { report, evidenceArtifactPath: SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT, evidenceBundle: bundle };
  } catch (error) {
    const journal = readJournal(SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL);
    ...
    if (!providerAttempted) {
      ...
      rmSync(SEMANTIC_GUARDIAN_V4_GATE_V2_JOURNAL, { force: true });
      return {
        report: blockedV4StandingReportV2(error?.message ?? String(error)),
        evidenceArtifactPath: null,
        evidenceBundle: null,
      };
    }
    ...
    const failedReport = {
      ...blockedV4StandingReportV2("The sealed standing gate terminated after a real provider attempt."),
      status: "failed",
    ...
    if (!existsSync(SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT)) writeBundle(SEMANTIC_GUARDIAN_V4_GATE_V2_ARTIFACT, bundle);
...
async function main() {
  ...
  if (options.summaryOnly) { process.stdout.write(formatStandingGateV2Summary(readBundle(options.evidencePath))); return; }

  const result = await runSealedSemanticGuardianV4GateV2();
  ...
  if (options.summary) process.stdout.write(formatStandingGateV2Summary(bundle));
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  main().catch((error) => {
    process.stderr.write(`Semantic Guardian v4 standing gate v2 failed: ${error?.message ?? String(error)}\n`);
    process.exitCode = 1;
  });
}
