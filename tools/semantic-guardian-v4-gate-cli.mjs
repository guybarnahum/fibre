...
function parseArgs(argv) {
  const options = {
    ...
    json: false,
    summaryOnly: false,
    evidencePath: SEMANTIC_GUARDIAN_V4_GATE_ARTIFACT,
    help: false,
  };
  ...
  if (!options.summaryOnly && options.evidencePath !== SEMANTIC_GUARDIAN_V4_GATE_ARTIFACT) {
...
function readJournal(path) {
  ...
  return text.split("\n").map((line, index) => {
    ...
    catch (error) { throw new Error(`standing-gate journal line ${index + 1} is invalid JSON: ${error.message}`); }
  });
}

function readBundle(path) {
  if (!existsSync(path)) throw new Error(`standing-gate evidence artifact not found: ${path}`);
...
function startProgress(path) {
  ...
  process.stderr.write(
    `\nFibre · Semantic Guardian v4 standing gate\n` +
  ...
  return {
    stop(status) {
      ...
      process.stderr.write(
        `\r\u001b[2K${symbol} Standing gate · ${snapshot.responses}/${EXPECTED_CASES} model responses · ${status} · ${elapsedLabel(Date.now() - startedAt)}\n\n`,
      );
    },
  };
}
...
export function formatStandingGateSummary(bundle) {
  ...
  lines.push(
    ...
    report.standingDifferentialGatePassed
      ? "The frozen v4 candidate passed its fresh held-out standing gate. This sealed artifact may support Semantic Guardian standing credit."
      : "The frozen v4 candidate did not pass this sealed standing gate. Do not tune or rerun this cycle; any new candidate requires a new frozen boundary and new held-out gate.",
  );
  return `${lines.join("\n")}\n`;
}

export async function runSealedSemanticGuardianV4Gate({ environment = process.env } = {}) {
  assertSemanticGuardianV4FrozenBoundary();
  if (existsSync(SEMANTIC_GUARDIAN_V4_GATE_ARTIFACT) || existsSync(SEMANTIC_GUARDIAN_V4_GATE_JOURNAL)) {
    return {
      report: blockedV4StandingReport(
        `Standing gate ${SET.id} is already sealed by an existing evidence artifact or journal. Never rerun a sealed cycle.`,
      ),
      evidenceArtifactPath: existsSync(SEMANTIC_GUARDIAN_V4_GATE_ARTIFACT) ? SEMANTIC_GUARDIAN_V4_GATE_ARTIFACT : null,
      evidenceBundle: null,
    };
  }

  mkdirSync(dirname(SEMANTIC_GUARDIAN_V4_GATE_JOURNAL), { recursive: true });
  const observer = (event) => appendFileSync(SEMANTIC_GUARDIAN_V4_GATE_JOURNAL, `${JSON.stringify(event)}\n`, "utf8");
  let runtime;
  let adapter;
  try {
    ...
    if (mismatches.length > 0) {
      rmSync(SEMANTIC_GUARDIAN_V4_GATE_JOURNAL, { force: true });
      return {
        report: blockedV4StandingReport(`Frozen runtime boundary mismatch: ${mismatches.join("; ")}`),
    ...
    if (error?.code === "MODEL_UNAVAILABLE") {
      return { report: blockedV4StandingReport(error.message), evidenceArtifactPath: null, evidenceBundle: null };
    }
    throw error;
  }

  const progress = startProgress(SEMANTIC_GUARDIAN_V4_GATE_JOURNAL);
  try {
    const report = await runSemanticGuardianV4StandingProof({ modelAdapter: adapter });
    const journal = readJournal(SEMANTIC_GUARDIAN_V4_GATE_JOURNAL);
    const bundle = bundleFor(report, journal);
    writeBundle(SEMANTIC_GUARDIAN_V4_GATE_ARTIFACT, bundle);
    rmSync(SEMANTIC_GUARDIAN_V4_GATE_JOURNAL, { force: true });
    progress.stop(report.status);
    return { report, evidenceArtifactPath: SEMANTIC_GUARDIAN_V4_GATE_ARTIFACT, evidenceBundle: bundle };
  } catch (error) {
    const journal = readJournal(SEMANTIC_GUARDIAN_V4_GATE_JOURNAL);
    ...
    if (!providerAttempted) {
      ...
      return {
        report: blockedV4StandingReport(error?.message ?? String(error)),
        evidenceArtifactPath: null,
        evidenceBundle: null,
      };
    }
    ...
    const failedReport = {
      ...blockedV4StandingReport("The sealed standing gate terminated after a real provider attempt."),
      status: "failed",
    ...
    if (!existsSync(SEMANTIC_GUARDIAN_V4_GATE_ARTIFACT)) writeBundle(SEMANTIC_GUARDIAN_V4_GATE_ARTIFACT, bundle);
    rmSync(SEMANTIC_GUARDIAN_V4_GATE_JOURNAL, { force: true });
...
async function main() {
  ...
  if (options.summaryOnly) { process.stdout.write(formatStandingGateSummary(readBundle(options.evidencePath))); return; }

  const result = await runSealedSemanticGuardianV4Gate();
  ...
  if (options.summary) process.stdout.write(formatStandingGateSummary(bundle));
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  main().catch((error) => {
    process.stderr.write(`Semantic Guardian v4 standing gate failed: ${error?.message ?? String(error)}\n`);
    process.exitCode = 1;
  });
}
