...
async function probeBinding(env, component) {
  ...
  try {
    const response = await binding.fetch(new Request("https://fibre.internal/healthz", { headers:{ Accept:"application/json" }, signal:AbortSignal.timeout(2500) }));
...
async function probeViewer(env, fetchImpl) {
  try {
    const origin = new URL(env.VIEWER_ORIGIN);
    const response = await fetchImpl(origin, { redirect:"follow", signal:AbortSignal.timeout(3000) });
...
export async function currentPublicStatus(env, { fetchImpl = globalThis.fetch, now = () => new Date().toISOString() } = {}) {
  const checks = await Promise.all([probeViewer(env, fetchImpl), ...COMPONENTS.map((component) => probeBinding(env, component))]);
