...
async function initialPresentationBundle() {
  ...
  media.generatedAt = presentation.manifest.generatedAt;
  provenance.threadId = THREAD_ID;
  provenance.generatedAt = presentation.manifest.generatedAt;
  provenance.entries = [
    ...provenance.entries.filter((entry) => entry.provenanceId !== "prov_slice_b_civil"),
    {
      ...
      sourceReferences:["registration_slice_b", "birth_slice_b", "world_slice_b"],
      note:"Authoritative civil identity projection for visual handoff proof.",
    },
  ];
...
test("admitted visual identity survives retry and converges through FID ensure", async () => {
  ...
  const reconciler = createThreadPresentationVisualPublicationReconciler({
    ...
    async ensureFid(input) {
      fidCalls.push(input);
      return fidCalls.length === 1
        ? { complete:false, state:"derivation_requested", derivation:{ jobId:"fid_photo_1" } }
        : { complete:true, state:"active", credential:{ credentialId:"fidc_slice_b", revision:1 } };
    },
  });
  ...
  assert.equal(first.stage, "fid_pending", "initial FID work was not retryable");

  const projected = await presentationServer.getSnapshot(CHANNEL_ID);
  assert.deepEqual(projected.snapshot.presentation.visualIdentity.referenceObjectRefs, [ROOT_OBJECT_REF]);

  const second = await boundary.reconcileAvailableEmbodiment({
    threadId:THREAD_ID,
    embodiment:visual,
    observedAt:"2026-08-31T00:51:00Z",
  });
  assert.equal(second.complete, true, "FID ensure did not converge after retry");
  assert.equal(second.detail.fidCredentialId, "fidc_slice_b");
  assert.equal(fidCalls[0].idempotencyKey, fidCalls[1].idempotencyKey, "retry changed FID issuance identity");
});
