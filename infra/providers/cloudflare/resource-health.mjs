...
function durableObjectUsage(groups) {
...
function checksFor({ d1, durableObjects, workers, limits }) {
  ...
  checks.push(Object.freeze({ kind:"durable_object_rows_read", resource:"durable-objects", value:durableObjects.rowsRead, limit:limits.durableObjectRowsReadDaily, level:level(durableObjects.rowsRead, limits.durableObjectRowsReadDaily) }));
  checks.push(Object.freeze({ kind:"durable_object_rows_written", resource:"durable-objects", value:durableObjects.rowsWritten, limit:limits.durableObjectRowsWrittenDaily, level:level(durableObjects.rowsWritten, limits.durableObjectRowsWrittenDaily) }));
...
export async function sampleCloudflareResourceHealth({
  ...
  const durableObjects = durableObjectUsage(account.durableObjectsPeriodicGroups ?? []);
  ...
  return Object.freeze({
    ...
    environment:env,
    observedAt:end,
    window:Object.freeze({ workersStart:start, workersEnd:end, dailyDate:date }),
