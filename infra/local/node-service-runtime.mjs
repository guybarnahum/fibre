...
export function createNodeServiceHandler({ runtime, maxBodyBytes = 1024 * 1024 } = {}) {
  if (!runtime || typeof runtime.fetch !== "function") {
    throw new TypeError("Node service adapter requires a service runtime");
  }
  ...
  return async function nodeServiceHandler(request, response) {
    try {
      ...
      await writeFetchResponse(await runtime.fetch(fetchRequest), response);
    } catch (error) {
