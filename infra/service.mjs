...
function normalizedRoute(route, index) {
  ...
  if (path === "/healthz") throw new TypeError("/healthz is reserved by the service layer");
...
async function healthPayload(serviceName, health) {
  const details = await health();
  plainObject("health result", details);
  if (Object.hasOwn(details, "ok") || Object.hasOwn(details, "service")) {
    throw new TypeError("health details must not override ok or service");
  }
  return { ok: true, service: serviceName, ...details };
}
...
export function createService({
  ...
  const resolveHealth = normalizeHealth(health);
  if (!Array.isArray(routes)) throw new TypeError("routes must be an array");
  if (fallback !== null && typeof fallback !== "function") throw new TypeError("fallback must be null or a function");
  ...
  async function invoke(handler, context) {
    try {
  ...
  return Object.freeze({
    ...
    async fetch(request) {
      if (!request || typeof request.url !== "string" || typeof request.method !== "string" || !request.headers) {
        throw new TypeError("service requires a Fetch Request");
      }
      const url = new URL(request.url);
      const method = request.method.toUpperCase();
      const context = { request, url, serviceName: normalizedServiceName };

      if (method === "GET" && url.pathname === "/healthz") {
        return jsonResponse(200, await healthPayload(normalizedServiceName, resolveHealth));
      }
      ...
      if (route) {
        ...
        return invoke(route.handler, context);
      }

      if (fallback !== null) return invoke(fallback, context);
