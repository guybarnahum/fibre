...
- Do not make a causal test's outcome depend on regexes, capitalization, exact prose fragments, prompt wording, or other incidental language unless that language is itself the semantic contract under test. Prefer stable domain facts, authority boundaries, IDs, enums, causal records, or controlled state changes; natural-language evidence may be asserted for presence/ownership without turning wording into a hidden API.
- Debug live Fibre behavior from evidence, never from guesswork. When a staging/runtime behavior is surprising, first add or use bounded inspection, diagnostic output, observatory data, or other direct evidence that reveals the actual inputs/decisions. Change behavior only after the evidence supports a cause. Temporary debug output must not silently become durable semantic authority or acceptance evidence.
...
- For focused Node tests, use `npm run test:focused -- ...` rather than bare `node --test ...`; Fibre test entrypoints default to `tools/test-infra/fibre-spec-reporter.mjs` so local and CI output stays compact. Use a different explicit reporter only when a task genuinely requires it.
