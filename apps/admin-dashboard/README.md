...
The Threads view is a **live World-presence map**, not a birthplace map. Inspecting the population selectively reconciles missing or stale LivedNow before the registry is reread, following Fibre's on-demand continuity model. Each plotted Thread is positioned from an authoritative CurrentSituation: the current place/venue comes from its resolved place reference, with the map using that place's World-locality coordinate; transit positions interpolate between resolved endpoints. A Thread whose present cannot be established is reported as current-location unavailable and is not silently plotted at its birthplace.
