...
The Threads view is a **live World-presence map**, not a birthplace map. Each admitted Thread is positioned from its authoritative CurrentSituation. The current place/venue comes from LivedNow's resolved place reference; the world projection uses that place's structured locality when available and otherwise the Thread's canonical World-locality coordinate. Transit positions interpolate between resolved endpoints. Legacy Threads that have not yet enacted LivedNow remain geographically visible at their known World locality but are explicitly labeled as awaiting enacted LivedNow rather than being misrepresented as an authoritative current venue.

Pending births expose operational age and staleness without creating a second lifecycle authority. After five minutes without state change, Birth Center distinguishes a Thread that is explicitly absent from World from a World lookup that is unavailable. If authoritative World already contains the stale Thread, Birth Center durably settles the birth as born/done and removes it from the active pipeline, including older Genesis-development reservations that predate the modern birth request surface. A pre-admission Pass-A validation failure is settled stillborn only when World explicitly confirms that no authoritative Thread exists. Ambiguous or unavailable World state remains visible for operator inspection rather than being guessed.
